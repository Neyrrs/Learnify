    ...
    public partial class UserRequest : UserControl
    {
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == dataGridView1.Columns["Aksi"].Index && e.RowIndex >= 0)
            {
                ...
                {
                    string bookingID = dataGridView1.Rows[e.RowIndex].Cells["id"].Value.ToString();
                    ...
                    if (result == DialogResult.Yes && con.State == ConnectionState.Closed)
                    {
                        ...
                        {
                            con.Open();
                            string query = "UPDATE Booking SET status = 'Canceled' WHERE id = @id";
        ...
        public void getData()
        {
            if (con.State == ConnectionState.Closed)
            {
                ...
                SELECT 
                    b.id AS id, 
                    p.nama AS NamaPengajar, 
                    b.status, 
                    b.tanggal_booking
                FROM Booking b
                JOIN Pengajar p ON b.pengajar_id = p.id
                WHERE b.status = 'Booked' OR b.status = 'Pending Cancel'";
